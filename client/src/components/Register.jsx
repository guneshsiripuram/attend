...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.role === 'student' && enrolledSamples.length < 3) {
      setError('Please complete all 3 face enrollment steps.');
      return;
    }

    setLoading(true);
    try {
      const face_descriptors = formData.role === 'student' ? enrolledSamples : null;
      ...
      if (googleUser) {
        await axios.post('/auth/complete-profile', {
          ...
          face_descriptor: face_descriptors
        });
      ...
      <div className="glass p-10 rounded-3xl shadow-2xl flex flex-col gap-8">
        <div className="text-center">
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Join the secure attendance portal
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className={`grid grid-cols-1 ${formData.role === 'student' ? 'md:grid-cols-2' : ''} gap-10`}>
          <div className="flex flex-col gap-6">
            ...
            {!googleUser ? (
              <>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-primary-500 outline-none"
              ...
              <div className="p-5 bg-primary-50 rounded-2xl border border-primary-100">
                <p className="text-[10px] font-black text-primary-500 uppercase flex items-center gap-2 mb-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-primary-500 outline-none uppercase"
            ...
            <div className="grid grid-cols-2 gap-4">
              <select
                className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-primary-500 outline-none cursor-pointer"
          ...
          <div className="flex flex-col gap-6">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2 flex justify-between items-center">
              ...
              <span className="text-primary-500">{enrolledSamples.filter(Boolean).length}/3 Steps</span>
            </h3>
            <div className="relative w-full aspect-square bg-slate-100 rounded-3xl overflow-hidden ring-4 ring-slate-50">
              <Webcam
                audio={false}
                ref={webcamRef}
              ...
              {!isCameraReady && !cameraError && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-100">
                  ...
                  <p className="text-sm font-bold text-slate-500">Initializing...</p>
                </div>
              )}
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2">
                <button
                  ...
                  className="py-4 px-8 rounded-2xl shadow-2xl bg-primary-600 text-white font-black text-sm uppercase flex items-center gap-3"
                >
                  {isRecording ? <Loader2 className="w-5 h-5 animate-spin" /> : <Camera className="w-5 h-5" />}
                  <span>{enrolledSamples[enrollmentStep] ? 'Retake' : 'Capture'} Step {enrollmentStep + 1}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-2 pt-6">
            <button
              type="submit"
              disabled={loading}
              className="w-full py-5 bg-primary-600 hover:bg-primary-700 text-white font-black rounded-2xl shadow-2xl transition-all"
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : (googleUser ? 'Activate Account' : 'Create Account')}
            </button>
          </div>
        </form>
      </div>
