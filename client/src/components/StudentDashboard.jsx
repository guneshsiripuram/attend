...
<<<<<<< HEAD
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          if (locCheck.data.success) {
            // STEP 2: Location OK -> Start Liveness Challenge
            startLivenessChallenge(loc);
          }
        } catch (err) {
          setResult({ 
            success: false, 
            message: err.response?.data?.message || 'LOCATION FAILED: Check your GPS permissions.' 
          });
      ...
      (err) => {
        setResult({ success: false, message: 'LOCATION FAILED: GPS access is required to mark attendance.' });
        setLivenessStatus('idle');
        setIsVerifying(false);
      },
      { timeout: 8000 }
    ...
    const livenessInterval = setInterval(async () => {
      ...
      if (frame) {
        ...
        if (FaceService.detectBlinkSequence(framesForLiveness)) {
          ...
          // STEP 3: Liveness OK -> Final Submission
          performFinalSubmit(loc);
        }
      }
      
      if (Date.now() - startLivenessTime > 15000) { // 15s timeout
        clearInterval(livenessInterval);
        if (livenessStatus !== 'success') {
          setLivenessStatus('idle');
          setResult({ success: false, message: 'IDENTITY FAILED: Blink timeout. Please ensure your face is well-lit and blink naturally.' });
          setIsVerifying(false);
        }
      }
    }, 150);
  ...
  const performFinalSubmit = async (loc) => {
    ...
    if (burstDescriptors.length < 3) {
      setResult({ success: false, message: 'IDENTITY FAILED: Poor image quality. Ensure your face is clearly visible.' });
      setIsVerifying(false);
      setLivenessStatus('idle');
      return;
    }
             ...
             {isVerifying && livenessStatus === 'checking_location' && (
               <div className="absolute inset-0 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center z-30">
                 <div className="flex flex-col items-center gap-4">
                    ...
                    <span className="text-white text-[10px] font-black uppercase tracking-widest">Verifying Campus Proximity...</span>
                 </div>
               </div>
             )}
