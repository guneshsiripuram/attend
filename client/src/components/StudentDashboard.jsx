...
const StudentDashboard = ({ user }) => {
  ...
  useEffect(() => {
    ...
    checkSession();
    const sessionInterval = setInterval(checkSession, 10000); // Check every 10s
    return () => clearInterval(sessionInterval);
  }, []);

  const handleVerify = async (manualImage = null) => {
    if (isVerifying || !isModelsLoaded || !isActuallyOpen) return;
  ...
  useEffect(() => {
    if (isAutoMode && !result?.success && !isVerifying && isActuallyOpen) {
  ...
  const isActuallyOpen = !!(session?.is_open && !isScheduled);

  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in relative">
      {/* Session Status Overlay for Closed Gate */}
      {!isSessionLoading && !isActuallyOpen && (
        <div className="absolute inset-0 z-50 bg-slate-100/10 backdrop-blur-[2px] rounded-3xl pointer-events-none flex items-center justify-center lg:hidden">
           {/* Mobile mini block if needed, but we handle it via disabled buttons mostly */}
        </div>
      )}
      ...
      <div className="lg:col-span-1 flex flex-col gap-8">
        <div className="glass p-6 rounded-2xl shadow-lg border border-white/50">
          ...
          <div className="h-64 relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              ...
              <span className="text-sm text-slate-500 font-medium">Overall</span>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 mt-4 text-center">
            <div className="p-3 bg-primary-50 rounded-xl border border-primary-100">
              <p className="text-xs text-primary-600 font-black uppercase tracking-wider">Present</p>
              <p className="text-xl font-bold text-slate-800">{stats.present} Days</p>
            </div>
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
              <p className="text-xs text-slate-500 font-black uppercase tracking-wider">Total Days</p>
              <p className="text-xl font-bold text-slate-800">{stats.total} Days</p>
            </div>
          </div>
        </div>

        <div className="glass p-6 rounded-2xl shadow-lg h-[450px] overflow-hidden flex flex-col border border-white/50">
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {history.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {history.map((log) => (
                  <div key={log.id} className="p-3.5 rounded-xl bg-white/60 border border-slate-100 flex items-center justify-between hover:bg-white transition-colors">
                    <div>
       ...
       <div className="lg:col-span-2 space-y-6">
         ...
         <div className={`p-4 rounded-2xl border-2 flex items-center justify-between transition-all duration-500 shadow-sm ${
           isActuallyOpen ? 'bg-green-50 border-green-100' : (isScheduled ? 'bg-amber-50 border-amber-100' : 'bg-red-50 border-red-100')
         }`}>
           <div className="flex items-center gap-4">
             <div className={`p-2.5 rounded-xl flex items-center justify-center shadow-lg ${isActuallyOpen ? 'bg-green-500 shadow-green-200' : (isScheduled ? 'bg-amber-500 shadow-amber-200' : 'bg-red-500 shadow-red-200')}`}>
               {isActuallyOpen ? <Shield className="w-5 h-5 text-white" /> : (isScheduled ? <Clock className="w-5 h-5 text-white" /> : <Lock className="w-5 h-5 text-white" />)}
             </div>
             <div>
                <p className={`text-sm font-black uppercase tracking-widest ${isActuallyOpen ? 'text-green-700' : (isScheduled ? 'text-amber-700' : 'text-red-700')}`}>
                  {isActuallyOpen ? 'Portal is OPEN' : (isScheduled ? 'Portal is SCHEDULED' : 'Portal is CLOSED')}
                </p>
         ...
         <div className="glass p-8 rounded-[2.5rem] shadow-2xl flex flex-col items-center gap-8 min-h-[600px] border border-white/50 relative overflow-hidden">
              ...
              <div className="absolute inset-0 z-40 bg-slate-900/5 backdrop-blur-[1px] flex items-center justify-center p-8 transition-opacity duration-500">
                 <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 max-w-sm text-center space-y-4 transform translate-y-[-20px]">
                    ...
                    <p className="text-sm text-slate-500 leading-relaxed">
                      {isScheduled ? (<>The portal is scheduled to open at <strong className="text-amber-600">{new Date(session?.starts_at || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</strong>. Please get your face ready.</>) : "Faculty hasn't opened the attendance portal yet. Please wait until the session begins."}
                    </p>
                 </div>
              </div>
            )}
           ...
           <div className="relative w-full max-w-lg aspect-[4/3] bg-slate-950 rounded-[2rem] overflow-hidden shadow-2xl ring-8 ring-white/80 group">
              <Webcam
                audio={false}
                ref={webcamRef}
                screenshotFormat="image/jpeg"
                className={`w-full h-full object-cover transition-opacity duration-700 ${!session.is_open ? 'opacity-40 grayscale' : 'opacity-100'}`}
                videoConstraints={{ facingMode: "user" }}
                onUserMedia={() => { setIsCameraReady(true); setCameraError(''); }}
                onUserMediaError={(err) => {
                  setIsCameraReady(false);
                  setCameraError('Camera access denied or device blocked.');
                }}
              />
                   ...
                   {isScheduled ? (
                     <div className="flex flex-col items-center justify-center">
                       ...
                       <p className="text-slate-400 text-xs font-bold text-center px-8 leading-relaxed">
                          ...
                          <span className="text-amber-500">{new Date(session?.starts_at || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>. Please be ready.
                       </p>
                     </div>
                   ) : (
                     <div className="flex flex-col items-center justify-center">
           ...
           <div className="w-full max-w-lg flex flex-col gap-6">
             ...
             {result && (
               <div className={`p-5 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top-2 duration-300 ${
                 result.success ? 'bg-green-50 border-2 border-green-100 text-green-700' : 'bg-red-50 border-2 border-red-100 text-red-700'
               }`}>
                 <div className={`p-2 rounded-xl ${result.success ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
             ...
             {result?.success && (
                <button 
                    ...
                    className="mx-auto text-primary-600 text-xs font-black uppercase tracking-widest hover:text-primary-800 transition-colors"
                >
